---
title: "Writing Basic Bounding Tests"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Writing Basic Bounding Tests}
...
`testhta` provides three key assertion functions to write these tests without needing custom comparison wrappers:
...
If the discount rate is extremely high (e.g. $1.0$, representing a $100\%$ discount rate), all future value is discounted to zero immediately. In `testhta`, a discount rate of 1 or greater sets all cycle values to 0. We expect total QALYs to equal 0.
...
With an infinite cost discount rate, all costs incurred after time 0 are discounted to zero. Because `testhta` treats discount rates $\ge 1$ as zeroing out all cycles, we assert:
